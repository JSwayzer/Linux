...
#autor: Johnny S.
#data de alteracao: Sat Feb 06 05:03:39 2021 - America/Sao Paulo
...
#Incluida funcao de baixar catalog e extrair todos os links do catalog
#Corrigido problema para verificar se arquivo ja existia, agora a verificacao eh feita por um comando ls dentro do if
clear
#setup destination directory
dw=/media/sf_D_DRIVE/y/c/

#The code below is used to download the catalog and extract all the pages inside it.
#wget https://boards.4channel.org/g/catalog -O arq.html

#cat arq.html | sed 's/,\"teaser\"/\n/g' | grep -v -E 'count|robots' | awk -F'"' '{print "https://boards.4chan.org/g/thread/" $4}' >> images-catalog.txt
...
for l in `cat images-catalog.txt` ;do
	...
	cat arq.html | \
		...
		sed 's/\ /+/g' | \
		grep -v 'a+data-utc' | \
		sed 's/is2\.4chan/i\.4cdn/g' | \
		grep -v 'loading=.' | \
		grep -v '_span.' > images.txt
